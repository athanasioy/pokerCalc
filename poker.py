...
def isTwoPair(myhand,table_cards):
    ...
    if len(cards) >=2 and not any([x>=3 for x in occurs]):
        ...
        cards.sort(key = FigstoNums, reverse = True)
        # print("Sorted Cards {}".format(cards))
...
def isFullHouse(myhand,table_cards):
    ...
    cards = list(dups.keys())
    occurs = list(dups.values())
    # print(dups,cards,occurs)
    if any([x>=3 for x in occurs]) and any([x<=2 for x in occurs]):
        if len(cards) >= 2 and sum(occurs) >= 5 and any([x>2 for x in occurs]):
            FullHouseIndex = occurs.index(3)
            twoPairIndex = occurs.index(2)
            # cards.sort(key = FigstoNums, reverse = True)
            ...
            # print(myhand,table_cards)
            return 700 + FigstoNums(cards[FullHouseIndex]) + FigstoNums(cards[twoPairIndex])
    else:
        return 0
...
def getStrongestHandProbability(myhand,tableCards):
    ...
    for comb in itertools.combinations(removeCardsfromDeck(myhand+tableCards,deck),2):
        ...
        for tcCombs in itertools.combinations(removeCardsfromDeck(myhand+tableCards+list(comb),deck),cards_left):
            ...
            if isWinning(myhand,list(comb),tableCards+list(tcCombs)):
                Win += 1
            else:
                LosingHands.append([list(comb),tableCards+list(tcCombs)])
            sumOfHands += 1
    winProbability = round(Win/sumOfHands,3)
    return winProbability, LosingHands
...
# WinProb, LosingH = getStrongestHandProbability(hand,flop)
# print(WinProb,LosingH)
